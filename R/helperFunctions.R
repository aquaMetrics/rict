...
# Produce dataframe with value of alkalinity in bounds [1.2, 366] as warning, and a message "Succ" or "Fail" to
# remove record from dataset for prediction
...
getLogAlkalinity <-
  function(hardness,
    ...
    # Case none provided
    if (is.null(hardness) &
        is.null(calcium) & is.null(conduct) & is.null(ALK)) {
      # MVP but.. or is.na(hardness) & is.na(calcium) etc etc
      msg <-
        "Fail: one of hard, Calc, cond, alk is null, bounds exceeded"
      alk_frame <- data.frame(log10(0), msg)
      colnames(alk_frame) <- c("log", "msg")
      return (alk_frame)
    }
    # Case provided, start with calculations
    if (!is.null(hardness) & (!is.na(hardness))) {
      # dont use is.na()
    ...
    if (!is.null(ALK) & (!is.na(ALK))) {
      ...
      if ((ALK < lower_b) | (ALK > upper_b)) {
        # Completely out of bounds, should fail with null value for "alt"
        msg <- "Fail: ALKALINITY bounds exceeded"
        alk_frame <-
          data.frame(ALK.LOG, msg) # A null value means nothing or zero
        colnames(alk_frame) <- c("log", "msg")
        return (alk_frame)
      }
      ...
      if (ALK >= lower_b & ALK <= upper_b) {
        if (ALK == lower_b | ALK == upper_b) {
          ...
          colnames(alk_frame) <-
            c("log", "msg") # past0 to "log" for name of attribute/parameter
...
getValidEnvInput <- function(env_var, lower_b, upper_b, envName) {
  ...
  if (is.null(env_var) |
      lower_b < 0 |
      is.null(upper_b)) {
    # Should fail with null value for "alt"
    msg <-
      paste("Fail: ", envName, " bounds exceeded with null values")
    env_frame <-
      data.frame(-999, msg) # A null value means nothing or -999
    colnames(env_frame) <- c("log", "msg")
    return (env_frame)
  }

  # First check Fail conditions Case 2
  if ((0 < env_var & env_var < lower_b) | (env_var > upper_b)) {
    # Completely out of bounds, should fail
    msg <- paste("Fail: ", envName, " bounds exceeded")
    env_frame <-
      data.frame(log10(env_var), msg) # A null value means nothing or zero
    colnames(env_frame) <- c("log", "msg")
    return (env_frame)
  }
  # Do the normal things
...
getAltitude <- function(alt, lower_b, upper_b) {
  ...
  if (is.null(alt) |
      is.na(alt)) {
    # Null cant be evaluated, return log10(0)
    ...
    if (alt < lower_lowb |
      ...
      if ((alt > lower_lowb &
           alt < lower_b) | (alt > upper_b & alt < upper_highb)) {
        # Deal with warn cases, otherwise normal cases
        ...
        # normal case
        alt_frame <-
          data.frame(log10(alt), msg)  # msg = "Succ"
        colnames(alt_frame) <- c("alt_log", "alt_msg")
        return(alt_frame)
      } # end else
    } # end else
...
getSubstrate <-
  function(bould_cob,
    ...
    if (TOTSUB < lower_b | TOTSUB > upper_b) {
      # Check bounds [97,103] for TOTSUB
      ...
      if (TOTSUB >= lower_b & TOTSUB <= upper_b) {
        MSUBST <-
          ((-7.75 * bould_cob) - (3.25 * pebbles_gr) + (2 * snd) + (8 * silt_cl)) / TOTSUB
        print(MSUBST)

        if (MSUBST < -8.0 | MSUBST > 8.0) {
          # Check if bounds broken, #hard coded for now
...
getSubstrate_old <- function(bould_cob, pebbles_gr, snd, silt_cl) {
  ...
  if (subst_total >= lower_b & subst_total <= upper_b) {
    return(((-7.75 * bould_cob) - (3.25 * pebbles_gr) + (2 * snd) + (8 * silt_cl)
    ) / subst_total)
  }
  else
    return(-999)
  ...
  function(nat_grid_ref,
    ...
    concat_bng  <-
      paste(nat_grid_ref,
            ...
            sep = "")
    lat_long    <-
      rnrfa::osg_parse(concat_bng, CoordSystem = coordsys_latlon)
    return(lat_long)
  }
...
getBNG <- function(nat_grid_ref,
  ...
  concat_bng <-
    paste(nat_grid_ref,
          ...
          sep = "")
  new_bng <-
    rnrfa::osg_parse(concat_bng, CoordSystem = coordsys_bng)
  return(new_bng)
}
...
getLogDischarge <-
  function(depth,
    ...
    if (is.na(discharge)) {
      # Case no discharge provided Check velocity
      if (!is.na(velocity) &
          !is.na(depth) & !is.na(width)) {
        # do calculations
        ...
        if (velocity < vel_lower | velocity > vel_upper_b) {
          ...
          if (velocity == vel_lower | velocity == vel_upper_b) {
            ...
            # normal velocity, do calculations
            disch <- depth / 100 * width * velocity / 100
            disch_frame <- data.frame(log10(disch), msg)
            colnames(disch_frame) <- c("log", "msg")
            return(disch_frame)
        ...
        # In case all are NA including depth, width and velocity
      ...
      # Normal service resumes, discharge not null and provided
