...
`rict` - produces single unnest output for predictions (metrics and taxa), classifications and comparisons.
`rict_predict` - predictions 
`rict_classify` - classification 
`rict_compare` - compare
...
### Significnt functions which are not exported 

compare_test - probability results are in same class
compare_pobabilities - probabilities of result A in result B classes
