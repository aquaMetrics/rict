...
* Using namespaces on some Non-standard evaluation functions can sometimes cause issues, for instance `dplyr::summarise(data, dplyr::n())` - the `dplyr::n()` won't work. 
...
```{r}
embed_png("images/rict-compare.png", dpi = 160)
```  
...
```{r}
knitr::kable(demo_observed_values[1, ])
```

NI - Model 1 input format

```{r}
knitr::kable(demo_ni_observed_values[1, ])
```
