...
![](images/rict_logo.png){ width=20%, align=center }
...
5. See [Prediction](#prediction) and  
...
The output from predictions always includes various 'book keeping' columns for reference:
...
predicted compared against observed values in the
[Classification](#classification) process. However, selecting the 'All indices'
option will predicted indices for a range of other possible ecological insights.
...
The inclusion of an index in the RICT ‘all indices' prediction must not be
regarded as an endorsement of that index. Some indices have been included to
...
Predict five taxa lists (TL) at different levels of taxonomic resolution: 
...
Compare allows you to run a comparison between two sites or the same site at
different times. The outputs consist of ASPT & NTAXA EQRs and their difference
...
There first version of RICT was a web application hosted by SEPA. This version
was written in Oracle Express. The second version of RICT was re-written in R
and hosted on Azure ML Studio. Re-writing in R made it more accessible to
researchers and other ecologists to modify and customise. This also makes the
software easier and cheaper to maintain.

## Changes Made

RICT version 2 corrected several errors in the original RICT, particularly the
prediction and classification programmes.  Because of this, results from RICT
version 2 differ from those produced by RICT. The following errors were
discovered in RICT and corrected in RICT version 2:

•	The RICT inadvertently used the ‘composite families’ version of WHPT ASPT (i.e. TL2 WHPT Score AbW,CompFam) rather than the ‘distinct families’ version (TL2 WHPT Score AbW,DistFam), although it did take account of the additional families included in WHPT.  The composite families are those that were grouped together in the BMWP-score in compliance with the current taxonomy of the time, outlined in Maitland (1977), see Appendix A
...
Results from RICT version 2 were checked against independent programmes using
...
Multi-year site assessments in RICT version 2 are based on averaging the EQR for
...
The latest input file can be obtained from the RICT site. Ensure you have
