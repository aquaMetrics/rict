...
calcRict <- function(environmentalVariables, endPointMeans) {
  ...
  taxaabDFrame <- read.csv("inst/extdat/TAXAAB.csv")

  #EndPoints <- read.csv("inst\extdat\Test_Data_End_Point_Means_Copy.csv")
  EndPoints <- EndPtMeans

  DFMean_gd <- read.delim("inst/extdat/DFMEAN_GB685.DAT", header = FALSE, sep="", as.is=TRUE)
  DFCoeff_gb685 <- read.delim("inst/extdat/DFCOEFF_GB685.DAT", header = FALSE, sep="", as.is=TRUE)
  ...
  BNG <- data.frame(easting = c(494200, 422000, 224600, 404800, 592100, 407400, 560400, 443700, 134400, 251500, 320300, 263000, 494200, 422000, 224600, 404800, 592100, 407400, 560400, 443700, 134400,
                                251500, 320300, 263000), northing = c( 491000,  364600,  111600,  325900,  227200,  110500,  297000, 1129000,  655200,  695500,  858400,  366300,  491000,  364600,  111600,  325900,  227200,  110500,
                                                                       297000, 1129000,  655200,  695500,  858400,  366300))
  ...
  lat_long <- data.frame(lat = c(54.30592, 53.17805, 50.87721, 52.83060, 51.91020, 50.89394, 52.54722, 60.04342, 55.71722, 56.12905, 57.60853, 53.17612, 54.30592, 53.17805, 50.87721, 52.83060,
                                 51.91020, 50.89394, 52.54722, 60.04342, 55.71722, 56.12905, 57.60853, 53.17612), lon = c(-0.5537803, -1.6722912, -4.4945403, -1.9301989,  0.7914189, -1.8961545,  0.3640949, -1.2173139, -6.2306341, -4.3907794, -3.3354977, -4.0512823, -0.5537803,
  ...
  raw.input.data$TMEAN <- c(9.486258,9.749915, 10.760873,  9.590491,10.098385,10.558048, 9.644871,7.530000, 9.427188, 8.757179,8.567249,10.224362,9.486258,9.749915,10.760873,9.590491,10.098385, 10.558048,9.644871,7.530000,9.427188,8.757179,8.567249,10.224362)
  raw.input.data$TRANGE <- c(11.953139, 12.562101,  9.848472, 12.650149, 13.669552, 12.495751, 13.584181,  8.880000,  9.090752, 12.241538, 11.206061, 10.298395, 11.953139, 12.562101,9.848472,12.650149, 13.669552, 12.495751, 13.584181, 8.880000,9.090752, 12.241538, 11.206061, 10.298395)
  ...
  #
  endroup_IndexDFrame <- read.csv("inst/extdat/END_GROUP_INDEX.csv", header = TRUE)
