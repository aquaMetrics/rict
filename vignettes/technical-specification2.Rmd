...
embed_png("images/application-flow-chart.png", dpi = 120)

```

**1. LOGIN TO RICT:** A new user will need to create a Microsoft account and log in to
...
**2. COMPLETE DATA INPUT:** User will need to complete a CSV (comma delimited) input
...
**9. COMPARE EXPERIMENT:** Tool runs processing steps to compare data outputs from two
previous predict and classify runs to produce a compare output file report.
  
**10. OUTPUT FILE:** RICT2 will generate an output file with processed data for Predict
...
Optional columns: Must still be present in input data file but they may contain missing values or be entirely empty. However, they must comply with the following notes:
...
```{r}
embed_png("images/rict-prediction.png", dpi = 120)
```
...
```{r}
embed_png("images/rict-classification.png", dpi = 120)
```  
...
```{r}
embed_png("images/rict-compare.png", dpi = 180)
```  
...
embed_png("images/rict-prediction-model-44.png", dpi = 160)
...
embed_png("images/rict-classification-model-44.png", dpi = 160)
