...
names_biological <- c("SPR_SEASON_ID", "SPR_TL2_WHPT_ASPT (ABW,DISTFAM)",
...
prob_names <- c("p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8", "p9", "p10", "p11", "p12", "p13", "p14", "p15", "p16",
...
while (k <= nrow(predictions) | (lastSiteProcessed == FALSE)) {
  ...
  while ((predictions[j, "SITE"] == siteToProcess  && j <= nrow(predictions))){
    ...
    # Afterwardds, use  EQR_ntaxa_aut <- rowMeans(multiYear_EQRAverages_ntaxa_aut[,-1])
  ...
  for (i in 1:5){
    probClass_spr[i] <- 100 * sum(classArray_siteOne_spr_aut_ntaxa[classArray_siteOne_spr_aut_ntaxa == i,
                                                                   ] / i) / n_runs
  }
  ...
  minta_ntaxa_aspt_spr_aut <- getMINTA_ntaxa_aspt(as.matrix(classArray_siteOne_spr_aut_ntaxa),
